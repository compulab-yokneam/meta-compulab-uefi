...
# SHA.
def sample_artifact_autorev_if_git_version(d):
...
SRCREV ?= '${@sample_artifact_autorev_if_git_version(d)}'
...
def sample_branch_from_preferred_version(d):
    ...
    if match is not None:
        ...
        return "master"
SAMPLE_ARTIFACT_BRANCH = "${@sample_branch_from_preferred_version(d)}"
...
PV = "${@sample_version_from_preferred_version(d, '${SRCPV}')}"

SRC_URI = "git://github.com/samplesoftware/sample-artifact.git;protocol=https;branch=${SAMPLE_ARTIFACT_BRANCH}"
...
LIC_FILES_CHKSUM = "file://src/github.com/samplesoftware/sample-artifact/LIC_FILES_CHKSUM.sha256;md5=${@sample_license(d.getVar('SAMPLE_ARTIFACT_BRANCH'))['md5']}"
LICENSE = "${@sample_license(d.getVar('SAMPLE_ARTIFACT_BRANCH'))['license']}"
