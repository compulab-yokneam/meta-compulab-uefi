...
# for vexpress-a9 board. See the script sample-qemu for an example of
...
python() {
    ...
    for varname in deprecated_vars:
        ...
        if cur:
            newvarname = varname.replace('SDIMG_', 'SAMPLE_')
...
sample_part_image() {
    ...
    if ${@bb.utils.contains('DISTRO_FEATURES', 'sample-uboot', 'true', 'false', d)}; then
    ...
    ondisk_dev="$(basename "${SAMPLE_STORAGE_DEVICE}")"

    wks="${WORKDIR}/sample-$suffix.wks"
    rm -f "$wks"
    if [ -n "${SAMPLE_IMAGE_BOOTLOADER_FILE}" ]; then
        # Copy the files to embed in the WIC image into ${WORKDIR} for exclusive access
        install -m 0644 "${DEPLOY_DIR_IMAGE}/${SAMPLE_IMAGE_BOOTLOADER_FILE}" "${WORKDIR}/"

        if [ $(expr ${SAMPLE_IMAGE_BOOTLOADER_BOOTSECTOR_OFFSET} % 2) -ne 0 ]; then
            ...
            bootloader_sector=$(expr ${SAMPLE_IMAGE_BOOTLOADER_BOOTSECTOR_OFFSET} + 1)
            bootloader_file=${WORKDIR}/${SAMPLE_IMAGE_BOOTLOADER_FILE}-partial
            dd if=${WORKDIR}/${SAMPLE_IMAGE_BOOTLOADER_FILE} of=$bootloader_file skip=1
        else
            bootloader_sector=${SAMPLE_IMAGE_BOOTLOADER_BOOTSECTOR_OFFSET}
            bootloader_file=${WORKDIR}/${SAMPLE_IMAGE_BOOTLOADER_FILE}
        fi
        ...
        if [ $bootloader_end -gt ${SAMPLE_UBOOT_ENV_STORAGE_DEVICE_OFFSET} ]; then
            bberror "Size of bootloader specified in SAMPLE_IMAGE_BOOTLOADER_FILE" \
                    "exceeds SAMPLE_UBOOT_ENV_STORAGE_DEVICE_OFFSET, which is" \
...
EOF
    ...
    if ${@bb.utils.contains('DISTRO_FEATURES', 'sample-uboot', 'true', 'false', d)} && [ -n "${SAMPLE_UBOOT_ENV_STORAGE_DEVICE_OFFSET}" ]; then
        boot_env_align_kb=$(expr ${SAMPLE_UBOOT_ENV_STORAGE_DEVICE_OFFSET} / 1024)
        cat >> "$wks" <<EOF
part --source rawcopy --sourceparams="file=${WORKDIR}/uboot.env" --ondisk "$ondisk_dev" --align $boot_env_align_kb --no-table
EOF
    fi

    if [ $(expr ${SAMPLE_PARTITION_ALIGNMENT} % 1024 || true) -ne 0 ]; then
        bbfatal "SAMPLE_PARTITION_ALIGNMENT must be KiB aligned when using partition table."
    fi

    alignment_kb=$(expr ${SAMPLE_PARTITION_ALIGNMENT} / 1024)
...
EOF
    ...
    if [ -n "${SAMPLE_IMAGE_BOOTLOADER_FILE}" ] && [ ${SAMPLE_IMAGE_BOOTLOADER_BOOTSECTOR_OFFSET} -ne $bootloader_sector ]; then
        # We need to write the first sector of the bootloader. See comment above
        # where bootloader_sector is set.
        dd if=${WORKDIR}/${SAMPLE_IMAGE_BOOTLOADER_FILE} of="$outimgname" seek=${SAMPLE_IMAGE_BOOTLOADER_BOOTSECTOR_OFFSET} count=1 conv=notrunc
    fi

    if [ -n "${SAMPLE_MBR_BOOTLOADER_FILE}" ]; then
        dd if="${DEPLOY_DIR_IMAGE}/${SAMPLE_MBR_BOOTLOADER_FILE}" of="$outimgname" bs=${SAMPLE_MBR_BOOTLOADER_LENGTH} count=1 conv=notrunc
    fi
    ...
    alignment=${SAMPLE_PARTITION_ALIGNMENT}
    ...
    if [ "$ptable_type" = "msdos" ]; then
        ...
        (
            ...
            echo ${SAMPLE_ROOTFS_PART_A_NUMBER}     # Number of partition
            echo 83                                 # "Linux filesystem" type

            echo t                                  # Partition type
            echo ${SAMPLE_ROOTFS_PART_B_NUMBER}     # Number of partition
            echo 83                                 # "Linux filesystem" type

            echo t                                  # Partition type
            echo ${SAMPLE_DATA_PART_NUMBER}         # Number of partition
            echo 83                                 # "Linux filesystem" type
    ...
    if ${@bb.utils.contains('DISTRO_FEATURES', 'sample-partuuid', 'true', 'false', d)}; then
        if [ "$ptable_type" = "gpt" ]; then
            # Set Fixed PARTUUID for all devices
            sgdisk -u ${SAMPLE_BOOT_PART_NUMBER}:${@sample_get_partuuid_from_device(d, '${SAMPLE_BOOT_PART}')} "$outimgname"
            sgdisk -u ${SAMPLE_ROOTFS_PART_A_NUMBER}:${@sample_get_partuuid_from_device(d, '${SAMPLE_ROOTFS_PART_A}')} "$outimgname"
            sgdisk -u ${SAMPLE_ROOTFS_PART_B_NUMBER}:${@sample_get_partuuid_from_device(d, '${SAMPLE_ROOTFS_PART_B}')} "$outimgname"
            sgdisk -u ${SAMPLE_DATA_PART_NUMBER}:${@sample_get_partuuid_from_device(d, '${SAMPLE_DATA_PART}')} "$outimgname"
        else
            diskIdent=$(echo ${@sample_get_partuuid_from_device(d, '${SAMPLE_ROOTFS_PART_A}')} | cut -d- -f1)
...
"
_SAMPLE_PART_IMAGE_DEPENDS += "${@bb.utils.contains('SAMPLE_DATA_PART_FSTYPE', 'btrfs','btrfs-tools-native:do_populate_sysroot','',d)}"


# This is needed because by default 'sample-grub' feature is used on ARM, but
...
do_image_uefiimg[depends] += "${_SAMPLE_PART_IMAGE_DEPENDS} \
                              gptfdisk-native:do_populate_sysroot"

do_image_biosimg[depends] += "${_SAMPLE_PART_IMAGE_DEPENDS}"
...
# Use the Sample part image as the Live image
python() {
...
sample_flash_mtdpart() {
    ...
    if [ "$size" = "-" ]; then
        # Remaining space.
        local total_space_kb=$(expr ${SAMPLE_STORAGE_TOTAL_SIZE_MB} \* 1024)
...
IMAGE_CMD_mtdimg() {
    ...
    ${@sample_make_mtdparts_shell_array(d)}
    ...
    while [ $i -lt $mtd_count ]; do
        ...
        if [ "$name" = "u-boot" ]; then
            if [ -n "${SAMPLE_IMAGE_BOOTLOADER_FILE}" ]; then
                sample_flash_mtdpart "${DEPLOY_DIR_IMAGE}/${SAMPLE_IMAGE_BOOTLOADER_FILE}" $size $kbsize $kboffset $name
            else
                bbwarn "There is a 'u-boot' mtdpart, but SAMPLE_IMAGE_BOOTLOADER_FILE is undefined. Filling with zeros."
                sample_flash_mtdpart "/dev/zero" $size $kbsize $kboffset $name
            fi
        elif [ "$name" = "u-boot-env" ]; then
            sample_flash_mtdpart "${DEPLOY_DIR_IMAGE}/uboot.env" $size $kbsize $kboffset $name
        elif [ "$name" = "ubi" ]; then
            sample_flash_mtdpart "${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.ubimg" $size $kbsize $kboffset $name
        else
