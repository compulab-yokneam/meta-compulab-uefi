...
get_part_number_from_device() {
    ...
    case "$1" in
        ...
        /dev/disk/by-partuuid/* )
            bberror "Please enable sample-partuuid Distro feature to use PARTUUID"
            ;;
    esac
...
sample_number_to_hex() {
    part_hex=$(printf "%X" $1 2>/dev/null)
}

def sample_get_partuuid_from_device(d, deviceName):
    import re
    if bb.utils.contains('DISTRO_FEATURES', 'sample-partuuid', True, False, d) and deviceName:
...
def sample_make_mtdparts_shell_array(d):
    ...
    if len(mtdparts) == 0:
        return "bbfatal 'SAMPLE_MTDPARTS is empty.'"

    active_mtdid = d.getVar('SAMPLE_IS_ON_MTDID')
    if len(active_mtdid) == 0:
        return "bbfatal 'SAMPLE_IS_ON_MTDID is empty. Please set it to the mtdid inside SAMPLE_MTDPARTS that you want Sample to use.'"
    ...
    # Pick the mtdid that matches SAMPLE_IS_ON_MTDID.
    ...
    if current_mtdparts is None:
        return ("bbfatal 'Cannot find a valid mtdparts string inside SAMPLE_MTDPARTS (\"%s\"), "
                + "corresponding to SAMPLE_IS_ON_MTDID (\"%s\").'"
    ...
    for component in mtdparts.split(":")[1].split(","):
        ...
        if match is None:
            return "bbfatal \"'%s' is not a valid mtdparts string. Please set SAMPLE_MTDPARTS to a valid value\"" % mtdparts

        # Make a shell array out of the mtdparts.
        if match.group(3) is None:
            offset = total_offset
        else:
            offset = sample_mtdparts_convert_units_to_bytes(match.group(3), match.group(4))
        ...
        if match.group(1) == '-':
            ...
            size = sample_mtdparts_convert_units_to_bytes(match.group(1), match.group(2))
...
def sample_mtdparts_convert_units_to_bytes(number, unit):
...
sample_get_clean_kernel_devicetree() {
    if [ -n "${SAMPLE_DTB_NAME_FORCE}" ]; then
        SAMPLE_DTB_NAME="${SAMPLE_DTB_NAME_FORCE}"
    else
        # Strip leading and trailing whitespace, then newline divide, and remove dtbo's.
        SAMPLE_DTB_NAME="$(echo "${KERNEL_DEVICETREE}" | sed -r 's/(^\s*)|(\s*$)//g; s/\s+/\n/g' | sed -ne '/\.dtbo$/b; p')"

        if [ -z "$SAMPLE_DTB_NAME" ]; then
            bbfatal "Did not find a dtb specified in KERNEL_DEVICETREE"
            exit 1
        fi
    fi

    DTB_COUNT=$(echo "$SAMPLE_DTB_NAME" | wc -l)

    if [ "$DTB_COUNT" -ne 1 ]; then
        SAMPLE_DTB_NAME="$(echo "$SAMPLE_DTB_NAME" | tail -1)"
        bbwarn "Found more than one dtb specified in KERNEL_DEVICETREE (${KERNEL_DEVICETREE}). Only one should be specified. Choosing the last one: ${SAMPLE_DTB_NAME}. Set KERNEL_DEVICETREE to the desired dtb file to silence this warning."
    fi
    ...
    SAMPLE_DTB_NAME="$(basename "$SAMPLE_DTB_NAME")"

    # Return.
    echo "$SAMPLE_DTB_NAME"
}


def sample_is_msdos_ptable_image(d):
    mptimgs = 'sample-image-sd sample-image-bios'
    return bb.utils.contains_any('DISTRO_FEATURES', mptimgs , True, False, d)


def sample_get_data_and_total_parts_num(d):
    data_pos = 3
    parts_num = 3

    boot_part_size = d.getVar('SAMPLE_BOOT_PART_SIZE_MB')
    if (boot_part_size and boot_part_size != '0'):
        data_pos += 1
        parts_num += 1

    swap_part_size = d.getVar('SAMPLE_SWAP_PART_SIZE_MB')
    if (swap_part_size and swap_part_size != '0'):
        data_pos += 1
        parts_num += 1

    extra_parts = d.getVar('SAMPLE_EXTRA_PARTS')
    ...
    if parts_num > 4 and sample_is_msdos_ptable_image(d):
        parts_num += 1
        if data_pos >= 4:
            data_pos += 1
    return data_pos, parts_num

def sample_get_data_part_num(d):
    data_num, _ = sample_get_data_and_total_parts_num(d)
    return data_num

def sample_get_total_parts_num(d):
    _, total_part_num = sample_get_data_and_total_parts_num(d)
    return total_part_num

def sample_get_extra_parts_offset(d):
    data_num = sample_get_data_part_num(d)
    total_part_num = sample_get_total_parts_num(d)
...
# from SAMPLE_BOOT_PART_MOUNT_LOCATION, and merge with the files from
...
sample_merge_bootfs_and_image_boot_files() {
    ...
    cp -al "${IMAGE_ROOTFS}/${SAMPLE_BOOT_PART_MOUNT_LOCATION}" "$W"
...
def get_extra_parts(d):
    final_parts = []
    partsflags = d.getVarFlags("SAMPLE_EXTRA_PARTS") or {}
    if partsflags:
        parts = (d.getVar('SAMPLE_EXTRA_PARTS') or "").split()
...
def get_extra_parts_fstab_opts(d, id = None):
    parts_fstab_flags = d.getVarFlags("SAMPLE_EXTRA_PARTS_FSTAB") or {}
    extra_parts_flags = (d.getVar('SAMPLE_EXTRA_PARTS') or "").split()
    if parts_fstab_flags:
        if id in parts_fstab_flags:
            split = parts_fstab_flags[id].split()
            elems = len(split)
            if elems < 2:
                bb.fatal("SAMPLE_EXTRA_PARTS_FSTAB[%s] is invalid. You need to provide parameters for fs_vfstype and fs_mntops (see 'man fstab')" % id)
            elif elems < 3:
...
def get_extra_parts_fstab(d):
    out = []
    extra_parts_offset = sample_get_extra_parts_offset(d)
    device = d.getVar('SAMPLE_STORAGE_DEVICE_BASE')
