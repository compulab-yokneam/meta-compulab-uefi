# Class that creates an UBI image with an Sample layout
...
IMAGE_CMD_ubimg () {
    ...
    if [ "${SAMPLE_BOOT_PART_SIZE_MB}" != "0" ]; then
        ubimg_fatal "Boot partition is not supported for ubimg. SAMPLE_BOOT_PART_SIZE_MB should be set to 0."
    fi
    ...
    if ${@bb.utils.contains("DISTRO_FEATURES", "sample-uboot", "true", "false", d)}; then
        ...
        local alignment=${SAMPLE_PARTITION_ALIGNMENT}
        if [ $(expr $uboot_env_vol_size % $alignment || true) -ne 0 ]; then
            bbfatal "U-Boot environment size is not an even multiple of SAMPLE_PARTITION_ALIGNMENT ($alignment)."
        fi
...
vol_id=0
vol_size=${SAMPLE_CALC_ROOTFS_SIZE}KiB
vol_type=dynamic
vol_name=rootfsa
...
vol_id=2
vol_size=${SAMPLE_DATA_PART_SIZE_MB}MiB
vol_type=dynamic
vol_name=data
...
vol_id=${SAMPLE_UBOOT_ENV_UBIVOL_NUMBER_1}
...
vol_id=${SAMPLE_UBOOT_ENV_UBIVOL_NUMBER_2}
vol_size=$uboot_env_vol_size
vol_type=dynamic
